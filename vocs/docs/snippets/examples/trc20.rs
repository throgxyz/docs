...
use tronz::{LocalSigner, ProviderBuilder, TronSigner, U256, TRONGRID_NILE};
...
async fn main() -> anyhow::Result<()> {
    ...
    let query_addr_str = std::env::var("TRON_ADDRESS")
        .expect("TRON_ADDRESS env var required (address to query)");
